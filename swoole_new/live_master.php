...
class Master implements MessageComponentInterface {
    ...
    public function __construct() {
        ...
        $this->ws = new swoole_client(SWOOLE_SOCK_TCP, SWOOLE_SOCK_SYNC);
    }
    ...
    private function sendToDispatcher($payload, ConnectionInterface $from)
    {
        ...
        if (!$this->ws->isConnected()) {
            try {
                $r = $this->ws->connect('127.0.0.1', 8312, 2);
                if (!$r) {
                    $this->g_logger->info('master2Dispatcher : connect failed');
                }
            } catch (Exception $e) {
        ...
        try {
            $this->g_logger->info('master2Dispatcher :'.json_encode($payload));
            $ret = $this->ws->send($this->packData(json_encode($payload)));
            if ($ret === false) {
    ...
    private function packData($payload)
    {
        $b = pack('N', strlen($payload)).$payload;
        $this->g_logger->info('msg len:'.strlen($payload).'payall:'.strlen($b));
        return $b;
    }
